...
const EmployeeDashboard = () => {
  ...
  const fetchPendingVisitors = async () => {
    try {
      const response = await axios.get("https://your-api.com/api/visitors/pending");
  ...
  const handleApproval = async (visitorId: number, status: "APPROVED" | "DENIED") => {
    try {
      await axios.post(`https://your-api.com/api/visitors/${visitorId}/approval`, { status });
      fetchPendingVisitors(); // Refresh list
    } catch (error) {
    ...
    <div>
      ...
      <ul>
        {pendingVisitors.map((visitor) => (
          <li key={visitor.id}>
            {visitor.fullName} ({visitor.purpose})
            <button onClick={() => handleApproval(visitor.id, "APPROVED")}>Approve</button>
            <button onClick={() => handleApproval(visitor.id, "DENIED")}>Deny</button>
          </li>
        ))}
      </ul>
    </div>
